...
export const EVENT_POOL: EventDef[] = [
  ...
  // ── Modern named ops (added from disinfo-terms CSV integration) ────────
  {
    id: 'storm-1516-surge',
    weight: 3,
    conditions: (s) => (s.upgrades['impersonation-1'] ?? 0) >= 5,
    headline: 'Storm-1516 drops a fake-whistleblower video. Your impersonation chain hooks the cycle.',
    precedent: 'Microsoft Threat Intelligence tracks Storm-1516 (GRU-linked) producing fake-whistleblower AI videos: 2024 Kamala Harris hit-and-run hoax, Tim Walz stolen-valor smear, Pelosi DUI narrative. Reported tie to GRU Unit 29155.',
    duration: 60,
  ...
  {
    id: 'hamilton68-exposure',
    weight: 4,
    conditions: (s) => s.resources.attention >= 200_000,
    headline: 'Alliance for Securing Democracy publishes a Hamilton 68 cluster naming your accounts.',
    precedent: 'Hamilton 68 dashboard (Alliance for Securing Democracy, 2017+) tracked Russian-influence-linked Twitter accounts. The 2022 "Twitter Files" critiqued the methodology — but the underlying account-cluster mapping technique is now standard practice across DFRLab, Graphika, EU DisinfoLab.',
    duration: 60,
    resource: 'attention',
    mult: 0.70,
    technique: 'discrediting',
  ...
  {
    id: 'bellingcat-investigation',
    weight: 3,
    conditions: (s) => s.resources.attention >= 500_000,
    headline: 'Bellingcat publishes a thread reverse-engineering your network. Receipts attached.',
    precedent: 'Bellingcat (2014+, founded by Eliot Higgins): open-source intelligence that has identified GRU officers behind Skripal poisoning, MH17 attribution, Navalny poisoning team. Their methodology — geolocation, EXIF data, leaked databases — is now the industry standard for ATTRIBUTION of disinfo ops.',
    duration: 60,
    resource: 'credibility',
    mult: 0.55,
    technique: 'discrediting',
  ...
  {
    id: 'ccdh-disinfo-dozen',
    weight: 4,
    conditions: (s) => (s.assets.wellnessInfluencer ?? 0) >= 3 || (s.upgrades['impersonation-2'] ?? 0) >= 3,
    headline: 'CCDH "Disinformation Dozen" report. Half the named accounts are yours. Press cycle follows.',
    precedent: 'Center for Countering Digital Hate (CCDH, 2021): identified 12 anti-vaccine actors producing 65% of anti-vax content online — including RFK Jr., Joseph Mercola, Sherri Tenpenny, Christiane Northrup. Platforms initially resisted; eventual partial takedowns.',
    duration: 60,
    resource: 'engagement',
    mult: 1.45,
    technique: 'impersonation',
  ...
  {
    id: 'doj-doppelganger-seizure',
    weight: 3,
    conditions: (s) => (s.assets.doppelganger ?? 0) >= 5,
    headline: 'DOJ seizes 32 of your cloned-news domains. Doppelganger infrastructure scrambling.',
    precedent: 'DOJ Doppelganger Domain Seizure (Sept 2024): Justice Department seized 32 domains used by Russian Doppelganger campaign to impersonate outlets like Washington Post, Fox News, Le Monde. Treasury sanctions on AEZA Group (Bulgarian bulletproof host) followed in 2025.',
    duration: 60,
    resource: 'engagement',
    mult: 0.60,
    technique: 'impersonation',
  ...
  {
    id: 'pravda-network-boost',
    weight: 3,
    conditions: (s) => s.phase === 'aisaturation' || s.resources.attention >= 5_000_000,
    headline: 'Pravda Network content crawls into the next LLM training set. The model now repeats your framing.',
    precedent: 'Pravda Network (Moscow, 2022+) seeds pro-Kremlin content across thousands of low-traffic sites to be ingested by web crawlers feeding LLM training data. American Sunlight Project / NewsGuard (2024-25) documented the practice; coined "LLM grooming."',
    duration: 60,
    resource: 'narrativeDominance',
    mult: 1.6,
    technique: 'discrediting',
  },
  {
